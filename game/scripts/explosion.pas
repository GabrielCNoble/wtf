...
void OnUpdate(particle_system_t @particle_system, array<vec4_t> @particle_positions, array<particle_t> @particles, array<int> @particle_frames)
{
	...
	for(i = 0; i < particle_count; i++)
	{
		...
		{
			particles[i].velocity.x = (randfloat() * 2.0 - 1.0) * 0.1;
			particles[i].velocity.y = 0.0;
			particles[i].velocity.z = (randfloat() * 2.0 - 1.0) * 0.1;
			particle_positions[i].w = 0.01;

			particle_frames[i] = 0;
		}

		if(particle_frames[i] < particle_system.max_frame)
		{
			if((particle_GetLife() % 2) == 0)
			{
				particle_frames[i]++;
			}
