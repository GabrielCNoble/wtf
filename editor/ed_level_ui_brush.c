...
void editor_BrushFacePropertiesWindowMaterialOptionListCallback(widget_t *widget)
{
	...
	if(widget->type == WIDGET_OPTION)
	{
		...
		{*/
		brush_face_properties_material_dropdown->widget.var->addr = &mat_material_names[ed_selected_brush->base_polygons[ed_selected_brush_polygon_index].material_index];
		//}
...
void editor_BrushFaceUVWindowSurfaceCallback(widget_t *widget)
{
	...
	if(widget->type == WIDGET_SURFACE)
	{
		...
		polygon = ed_selected_brush->base_polygons + ed_selected_brush_polygon_index;
		...
		{	
			...
			for(i = ed_selected_brush_selection_index + 1; i < ed_selection_count; i++)
			{
				polygon->vertices[ed_selections[i].index0].tex_coord.x += dx;
				polygon->vertices[ed_selections[i].index0].tex_coord.y += dy;
			}
		...
		//if(ed_selected_brush_polygon_index > -1)
		{
			...
			for(i = ed_selected_brush_selection_index + 1; i < ed_selection_count; i++)
			{
				...
				pos.x = polygon->vertices[ed_selections[i].index0].tex_coord.x * 2.0 - 1.0 + view_offset_x;
				pos.y = polygon->vertices[ed_selections[i].index0].tex_coord.y * 2.0 - 1.0 + view_offset_y;
				pos.z = -0.5;
				pos.w = 1.0;
...
void editor_OpenBrushFacePropertiesWindow(int brush_index, int face_index)
{
	...
	if(brush_face_properties_window)
	{
		...
		for(i = -1; i < mat_material_count; i++)
		{
			...
			option = gui_AddOptionToList(brush_face_properties_material_list, mat_material_names[i], mat_material_names[i]);
		...
		{*/
		brush_face_properties_material_dropdown->widget.var->addr = &mat_material_names[brush->base_polygons[face_index].material_index];
		//}
