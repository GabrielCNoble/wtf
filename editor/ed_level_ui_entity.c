...
extern struct entity_def_t *ent_entity_defs;
...
void editor_InitEntityUI()
{
	...
	mat3_t_rotate(&thumbnail_camera_orientation, vec3_t_c(1.0, 0.0, 0.0), -0.2, 1);
	mat3_t_rotate(&thumbnail_camera_orientation, vec3_t_c(0.0, 1.0, 0.0), 0.25, 0);
	
	thumbnail_camera = camera_CreateCamera("thumbnail camera", thumbnail_camera_position_vector, &thumbnail_camera_orientation, 0.5, ENTITY_DEF_THUMBNAIL_WIDTH, ENTITY_DEF_THUMBNAIL_HEIGHT, 0.1, 100.0, 0);
...
void editor_UpdateThumbnail(widget_t *thumbnail)
{
	...
	glLoadIdentity();
	glTranslatef(0.0, -mesh->aabb_max.y, 0.0);
	glMultMatrixf(&camera->world_to_camera_matrix.floats[0][0]);
	...
	if(model)
	{
		...
		{
			renderer_SetMaterial(model->batches[i].material_index);
			glDrawArrays(GL_TRIANGLES, model->vert_start + model->batches[i].start, model->batches[i].next);
		}
...
{
	mat3_t id = mat3_t_id();
//	entity_CreateEntity("entity", ed_3d_cursor_position, vec3(1.0, 1.0, 1.0), &id, entity_def_index);
}
