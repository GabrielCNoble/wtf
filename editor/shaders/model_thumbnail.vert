...
void main()
{
	...
	world_space_normal = vec3(UNIFORM_model_matrix * vec4(vertex_normal.xyz, 0.0));
	world_space_tangent = vec3(UNIFORM_model_matrix * vec4(vertex_tangent.xyz, 0.0));
	world_space_position = vec3(UNIFORM_model_matrix * vertex_position);
	eye_space_position = vec3(UNIFORM_model_view_projection_matrix * vertex_position);
