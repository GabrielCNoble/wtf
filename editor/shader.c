...
int shader_LoadShader(char *file_name)
{
	...
	if(!(f = fopen(vertex_shader_file_name, "r")))
	{
	...
	if(!status)
	{
		...
		printf("Error compiling %s vertex shader!\n%s\n", file_name, error_log);
		free(error_log);
	...
	if(!(f = fopen(fragment_shader_file_name, "r")))
	{
...
{	
	switch(uniform)
	{
		...
		/*case UNIFORM_LIGHT_COUNT:
			uniform = shaders[active_shader].light_count;
		break;*/
