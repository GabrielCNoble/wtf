...
struct entity_t *entity_editor_current_entity_def = NULL;
collider_def_t *entity_editor_current_collider_def = NULL;
...
void editor_EntityEditorInit()
{
	...
	entity_editor_camera = camera_CreateCamera("entity editor camera", vec3(0.0, 0.0, 0.0), &orientation, 0.68, r_window_width, r_window_height, 0.1, 100.0, 0);
...
void editor_EntityEditorSetup()
{
	...
	entity_editor_light_index = light_CreateLight("entity editor light", &light_orientation, vec3(0.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0), 10.0, 10.0, 0);
...
void editor_EntityEditorMoveCamera()
{
	...
	else
	{
		...
		else*/
		{
			...
			mat3_t_rotate(&orientation, vec3(1.0, 0.0, 0.0), entity_editor_camera_v_angle, 1);
			mat3_t_rotate(&orientation, vec3(0.0, 1.0, 0.0), entity_editor_camera_h_angle, 0);
		}
...
void editor_EntityEditorEdit()
{
	...
	if(input_GetKeyStatus(SDL_SCANCODE_A) & KEY_JUST_PRESSED)
	{
		if(lshift)
		{
			editor_EntityEditorOpenAddComponentMenu(r_window_width * 0.5 * normalized_mouse_x, r_window_height * 0.5 * normalized_mouse_y);
