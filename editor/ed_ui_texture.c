...
void editor_EnumerateTextures()
{
	...
	for(i = 0; i < texture_count; i++)
	{
		...
		option = gui_AddOptionToList(texture_window_texture_list, texture_info[i].name, texture_info[i].name);
...
void editor_OpenTexturePropertiesWindow(int texture_index)
{
	if(texture_properties_window)
	{
		...
		texture_properties_window_texture_name_text_field->widget.var->addr = &texture_info[texture_index].name;
		texture_properties_window_invert_texture_x_checkbox->widget.var->addr = &textures[texture_index].bm_flags;
		texture_properties_window_invert_texture_y_checkbox->widget.var->addr = &textures[texture_index].bm_flags;
